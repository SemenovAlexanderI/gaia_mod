...
def instrumented_agent(
    ...
    **kwargs: Unpack[BasicAgentDeprecatedArgs],
    ...
    # main agent loop
    @solver
    def instrumented_agent_loop() -> Solver:
        async def solve(state: TaskState, generate: Generate) -> TaskState:
            ...
            state = await registry.trigger(BP.BEFORE_GENERATE, state)
            ...
            with create_token_limit(token_limit):
                ...
                while not state.completed:
                    ...
                    if state.output.message.tool_calls:
                        state = await registry.trigger(BP.AFTER_TOOL_CALL, state)
                        ...
                        state = await registry.trigger(BP.AFTER_TOOL_RESULT, state)
                ...
                state = await registry.trigger(BP.AFTER_FINAL_ANSWER, state)
            return state
